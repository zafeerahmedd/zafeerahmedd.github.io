...
(() => {
  ...
  const names=['Motorcycle Assembly','Stair-Climbing Robot','CFD & Heat-Transfer Studies','Terra Explorer Rover','Electric Go-Kart','Solar Desalination Plant','Custom FDM Printer','Kinematic Gear-Drive Vehicle'];
  ...
  function paint(s,announce=false){
    ...
    if(projectPicker)projectPicker.value=String(active);
    document.querySelector('#tooth-count').textContent=String(active+1).padStart(2,'0')+' OF 08';
    document.querySelector('.tooth-track i').style.width=((active+1)/8*100)+'%';
    document.querySelector('#tooth-hint').textContent=active===7?'Last project · keep scrolling to continue ↓':'Scroll to bring the next tooth into view ↓';
    prev.disabled=active===0;next.textContent=active===7?'Continue ↓':'→';next.setAttribute('aria-label',active===7?'Continue to all projects':'Next project');
    if(announce)document.querySelector('#tooth-status').textContent=`Project ${active+1} of 8: ${names[active]}`;
  }
  ...
  /* ---------------------------------------------------------------------
     Scroll chooses WHICH project is active. The retract / turn / expand is
     then animated on a clock, not scrubbed by the wheel, which is what made
     it feel stepped: a coarse wheel notch used to cover a whole transition.
     The three phases deliberately OVERLAP and use an expo-out curve, so the
     plate is already turning before it has finished closing.
     --------------------------------------------------------------------- */
  const DUR=460;
  const expoOut=t=>t>=1?1:1-Math.pow(2,-10*t);
  const inOut=t=>t<.5?4*t*t*t:1-Math.pow(-2*t+2,3)/2;
  const sub=(t,a,b)=>clamp((t-a)/(b-a),0,1);
  let fromIdx=0,toIdx=0,renderedIdx=0,animStart=0,animating=false,settled=true;
  function targetIndex(){
    const b=bounds();
    return clamp(Math.floor(clamp((scrollY-b.start)/b.range,0,0.99999)*8),0,7);
  }
  function tick(now){
    animating=false;lastTickAt=now;
    if(!story||!orbit())return;
    const t=clamp((now-animStart)/DUR,0,1);
    const closing=1-expoOut(sub(t,0,.34));
    const opening=expoOut(sub(t,.62,1));
    const turn=inOut(sub(t,.26,.74));
    const position=fromIdx+(toIdx-fromIdx)*turn;
    const docked=Math.abs(position-Math.round(position))<1e-6;
    const open=fromIdx===toIdx?1:(t<.5?closing:opening);
    const idx=t<.5?fromIdx:toIdx; renderedIdx=idx;
    paint({position:docked?Math.round(position):position,
           index:idx,
           open:docked?open:0,
           progress:toIdx/8,complete:toIdx===7});
    if(t<1){animating=true;requestAnimationFrame(tick);}
    else{fromIdx=toIdx;settled=true;}
  }
  function retarget(i){
    if(i===toIdx&&settled)return;
    if(i===toIdx)return;
    /* Restart from what is actually on screen, not from the previous tween's
       origin: otherwise a retarget mid-flight can show an index it has already
       passed, and the down-scroll sequence stops being monotonic. */
    fromIdx=settled?toIdx:renderedIdx;
    toIdx=i;settled=false;animStart=performance.now();
    if(motionOff()){fromIdx=toIdx;renderedIdx=toIdx;settled=true;paint({position:toIdx,index:toIdx,open:1,progress:toIdx/8,complete:toIdx===7});return;}
    if(!animating||performance.now()-lastTickAt>400){animating=true;lastTickAt=performance.now();requestAnimationFrame(tick);}
  }
  function updateScroll(){
    ...
    retarget(targetIndex());
  }
  let scheduledAt=0,lastTickAt=0;
  /* `pending` and `animating` are latches guarding a queued animation frame. A
     bfcache restore (browser Back) or a frozen background tab can drop that frame
     without ever running the callback, which used to leave the latch stuck true:
     the gear then ignored every later scroll. Both now expire. */
  function schedule(){
    const now=performance.now();
    if(pending&&now-scheduledAt<400)return;
    pending=true;scheduledAt=now;requestAnimationFrame(updateScroll);
  }
  function configure(){
    ...
    paint({position:active,index:active,open:1,progress:active/8,complete:false});
    if(enabled)schedule();
  }
  ...
  function choose(index){
    index=clamp(index,0,7);fromIdx=toIdx;toIdx=index;renderedIdx=index;settled=true;paint({position:index,index,open:1,progress:index/8,complete:false},true);
    if(orbit()){const b=bounds();scrollInstant(b.start+b.range*(index+.2)/8);}
    else if(projectPicker){
      // Keep the compact picker and its changed image together in the viewport.
      requestAnimationFrame(()=>{
        const picker=projectPicker.closest('.compact-select');
        const header=document.querySelector('.header');
        const offset=(header?.getBoundingClientRect().height||80)+12;
        const top=picker.getBoundingClientRect().top;
        if(top<offset||top>offset+24)scrollInstant(scrollY+top-offset);
      });
    }
  }
  if(story){
    ...
    projectPicker?.addEventListener('change',()=>choose(Number(projectPicker.value)));
    prev.addEventListener('click',()=>choose(active-1));
    next.addEventListener('click',()=>{if(active===7)(document.querySelector('#all-projects')||document.querySelector('#experience')).scrollIntoView({behavior:motionOff()?'auto':'smooth'});else choose(active+1);});
    compactButton.addEventListener('click',()=>{compact=!compact;configure();const b=bounds();scrollInstant(orbit()?b.start+b.range*(active+.2)/8:b.start);});
  ...
  /* Coming back from a project page, the browser restores the old scroll position
     but the gear may still be showing whatever it showed when the page was frozen.
     Clear the latches and snap the wheel to whatever the scroll position now means. */
  function resync(){
    pending=false;animating=false;
    if(!story)return;
    fitFallback=false;configure();
    if(!orbit())return;
    const i=targetIndex();
    fromIdx=toIdx=renderedIdx=i;settled=true;
    paint({position:i,index:i,open:1,progress:i/8,complete:i===7});
    schedule();
  }
  addEventListener('pageshow',e=>{if(e.persisted)resync();});
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)resync();});
  motionButton.addEventListener('click',()=>{const inside=story&&story.getBoundingClientRect().top<100&&story.getBoundingClientRect().bottom>100;userMotionOff=!userMotionOff;try{localStorage.setItem('zaq-motion',userMotionOff?'off':'on');}catch{}configure();if(inside){const b=bounds();scrollInstant(orbit()?b.start+b.range*(active+.2)/8:b.start);}});
  ...
  if(dialog){
    document.querySelectorAll('.gal img,.proof img').forEach(img=>{
      ...
      button.addEventListener('click',()=>{lastFocus=button;dialog.querySelector('img').src=img.src;dialog.querySelector('img').alt=img.alt;dialog.querySelector('p').textContent=img.closest('figure,.proof')?.querySelector('figcaption,.proof-label')?.textContent||img.alt;dialog.showModal();body.style.overflow='hidden';dialog.querySelector('button').focus();});
    });
...
(function(){
  function paintBg(img){
    const host=img.closest('.gal figure')||img.closest('.tooth-content figure')||img.closest('.hero-image');
    if(!host)return;
    const url=img.currentSrc||img.getAttribute('src');
    if(url)host.style.setProperty('--bgimg',`url("${url}")`);
    /* Size the frame to the picture. Only when the picture's shape falls outside
       what the layout can give it does the blurred backdrop get used at all. */
    const w=img.naturalWidth,h=img.naturalHeight;
    if(w&&h){
      const ar=w/h, lo=0.80, hi=2.30;
      const use=Math.min(hi,Math.max(lo,ar));
      host.style.setProperty('--ar',use.toFixed(4));
      host.classList.toggle('fits', Math.abs(use-ar)<0.02);
    }
  }
  function all(){document.querySelectorAll('.gal img,.tooth-content figure img,.hero-image img').forEach(i=>{
    paintBg(i);                                   /* the src attribute is enough */
    if(!i.complete)i.addEventListener('load',()=>paintBg(i),{once:true});});}
  all();
  ...
  document.querySelectorAll('.gal img,.tooth-content figure img,.hero-image img').forEach(i=>mo.observe(i,{attributes:true}));
})();
...
/* ---- all-project index: filtering, and the copy-email control ---- */
(function(){
  const grid=document.querySelector('.pj-grid');
  if(grid){
    const cards=[...grid.querySelectorAll('.pj-card')];
    const buttons=[...document.querySelectorAll('.pj-filter')];
    const shown=document.querySelector('#pj-shown');
    const status=document.querySelector('#pj-status');
    /* Cards are visible in the markup; JS only ever hides. With JS off every
       project stays on the page and every case-study link still works. */
    function apply(tag){
      let n=0;
      cards.forEach(c=>{const on=tag==='all'||c.dataset.tag===tag;c.hidden=!on;if(on)n++;});
      buttons.forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.filter===tag)));
      if(shown)shown.textContent=String(n).padStart(2,'0');
      const label=(buttons.find(b=>b.dataset.filter===tag)||{textContent:tag}).textContent;
      if(status)status.textContent=`${n} project${n===1?'':'s'} shown, filtered by ${label}`;
    }
    buttons.forEach(b=>b.addEventListener('click',()=>apply(b.dataset.filter)));
    /* The card header is a real button so keyboard and touch reach the same
       content hover gives a mouse. Hover state is CSS only; this is the click path. */
    cards.forEach(c=>{
      const head=c.querySelector('.pj-head'); if(!head)return;
  ...
  if(copy){
    ...
    copy.addEventListener('click',async()=>{
      ...
      try{
        ...
        copy.dataset.state='done';copy.textContent='Copied ✓';
      }catch{
        /* clipboard is blocked in some contexts; the address stays selectable beside it */
        copy.dataset.state='fail';copy.textContent='Press Ctrl+C to copy';
      }
      timer=setTimeout(()=>{copy.dataset.state='idle';copy.textContent=original;},2600);
    });
  }
})();

/* ---- scroll progress bar and scroll reveal ---- */
(function(){
  ...
  /* Elements are visible in the markup and only get .reveal once JS is running,
     so with JS off or the observer missing nothing is ever left hidden. */
  const reduce=matchMedia('(prefers-reduced-motion: reduce)');
  const off=()=>document.body.classList.contains('motion-off')||reduce.matches;
  if(!('IntersectionObserver' in window)||off())return;
  const targets=[...document.querySelectorAll(
    '.exp-strip>div,.role-detail,.pj-card,.skill,.credlist>li,.section-head,.about-stats>div,.marquee')];
  ...
  /* threshold must stay 0: an element taller than the viewport can never reach a
     fractional threshold, and would sit at opacity 0 forever. */
  ...
  let pending=false;
  addEventListener('scroll',()=>{if(!pending){pending=true;
    requestAnimationFrame(()=>{pending=false;sweep();});}},{passive:true});
})();

/* Copy the visible certificate ID; verification stays on the official issuer site. */
(function(){
  document.querySelectorAll('[data-certificate-id]').forEach(button=>{
    ...
    button.addEventListener('click',async()=>{
      clearTimeout(timer);
      try{
        await navigator.clipboard.writeText(button.dataset.certificateId);
        button.textContent='ID copied ✓';
      }catch{
        button.textContent=button.dataset.certificateId;
        const range=document.createRange();range.selectNodeContents(button);
        const selection=getSelection();selection.removeAllRanges();selection.addRange(range);
      }
      timer=setTimeout(()=>{button.textContent='Copy certificate ID';},3000);
    });
  });
})();
